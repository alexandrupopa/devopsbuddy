...
public class PasswordResetTokenIntegrationTest extends AbstractIntegrationTest {
	...
	@Test
	public void findTokenByTokenValue() throws Exception {
	...
	public void testMultipleTokensAreReturnedWhenQueringByUserId() throws Exception {
		...
		List<String> actualAsList = actualTokens.stream().map(prt -> prt.getToken()).collect(Collectors.toList());
		Assert.assertEquals(tokensAsList, actualAsList);
